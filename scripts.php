var rating_initial_value_bananas = "0"; 
...
jQuery.validator.addMethod("phonedefault",function(a,b){return this.optional(b)||/^(\+)?[0-9]+$/i.test(a)},"Please provide valid phone number");
...
function loading_message(message) {
    $("<div class='loading_custom'></div>").html(message).hide().show().prependTo("div#header");
...
$(function () {
    ...
    $("#change_user_location").on("click", function (e) {
        ...
        var $html = '<div style="left:' + (e.pageX - 240) + 'px" id="change_location">&nbsp; <strong>Zip: &nbsp;</strong> <input type="text" value="" /> <a href="javascript:void(0);" class="generalCloseBTN">x</a> <button class="actionbutton btn" name="change">CHANGE</button></div>';
    ...
    $("#change_user_location2").on("click", function (e) {
        ...
        var $html = '<div style="left:' + (e.pageX - 240) + 'px; top:' + e.pageY + 'px" id="change_location2">&nbsp; <strong>Zip: &nbsp;</strong> <input type="text" value="" /> <a href="javascript:void(0);" class="closeBTN">x</a> <button class="actionbutton btn" name="change">CHANGE</button></div>';
        $($html).appendTo("body").show(500, function () {
    ...
    if ($('body').hasClass('body-go_premium')) { // go premium page
        ...
        $(".selectduration").on("click", function () {
            ...
            if (selected_radio) {
                ...
                $("body").removeClass("selected-premium-first_choice selected-premium-second_choice selected-premium-third_choice selected-premium-bitcoin1 selected-premium-bitcoin2 selected-premium-bitcoin3 selected-premium-bitcoin4 selected-premium-ethereum1 selected-premium-ethereum2 selected-premium-ethereum3 selected-premium-ethereum4 selected-premium-tether1 selected-premium-tether2 selected-premium-tether3 selected-premium-tether4 selected-premium-gift1 selected-premium-gift2");
                $("body").addClass("selected-premium-" + selected_radio);

                // check / uncheck fake radio from cryptocurrency
                var credit_card_options = ["first_choice", "second_choice", "third_choice"];
...
$(document).ready(function () {
    ...
    $(document).on("click", ".verify_profile_intro .btn", function () {
        ...
        if (_showDiv == '.verify_profile_video') {
            ...
            $(_showDiv).find('.iframegoeshere').html('<iframe src="' + iframeURL + '" width="100%" height="470px" style="margin:0; padding:0; border:0; "></iframe>');
        }
    ...
    $("#login_form").validate({
        ...
        submitHandler: function (form) {
            ...
            let data = "login=1&email=" + $("#login_form input[name=email]").val() + "&password=" + $("#login_form input[name=password]").val();
            const mfaCode = $("#login_form #mfa_code_container #mfa_code").val();
            if(mfaCode) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data === 'loggedin') {
                        ...
                        $("#InputEmail").after("<label class=\"register_error register_error_email\" for=\"InputEmail\"><span></span>Your account is banned!</label>");
                    } else if (data === 'invalid_credentials') {
                        $("#login_form .register_error").remove();
                        $("#InputEmail").after("<label class=\"register_error register_error_email\" for=\"InputEmail\"><span></span>The email/username or password is incorrect</label>");
                    } else if (data === 'notconfirmed') {
                        $("#login_form .register_error").remove();
                        $("#InputEmail").after("<label class=\"register_error register_error_email\" for=\"InputEmail\"><span></span>Please check the registration email and confirm your account!  <a href=\"/users-resend-confirm\" class=\"btn\">Resend confirmation token</a></label>");
                    } else if (data === 'mfa_missing' || data === 'mfa_wrong') {
                        $("#login_form .register_error").remove();
                        $("#login_form #mfa_code_container").show();
                        const text = data === 'mfa_missing' ? 'Mfa code is missing.' : 'Mfa code is wrong.';
                        $("#login_form #mfa_code_container #form-error-mfa_wrong").html(`${text}`);
                    }
                }
            });
            return false;
    ...
    (function change_location_select() {
        ...
        button.click(function () {
            ...
            //search_input.after("<input type='hidden' name='state_country' value='" + $("#select_countries").val() + "' class='location_inputs'>");
            search_input.after("<input type='hidden' name='city_id' value='" + $("#select_city").val() + "' class='location_inputs'>");
            //search_input.after("<input type='hidden' name='zone_id' value='" + $("#select_zone").val() + "' class='location_inputs'>");
    ...
    $(".remove_csv_item").on("click", function () {
        ...
        $("<div style='text-align:center; font-size:16px;'>Loading...</div>").insertAfter($(".refine_search_btn"));
    ...
    $("#form_add_comment_to_review input[name='submit_comment']").on("click", function (e) {
        ...
        if (!$isOK) {
            ...
            $(".notice_error").remove();
            $("#comment-to-review").after('<div class="notice_error" style="clear:both;">' + error_to_display + '</div>');
            ...
            $("<div style='float:right; font-size:16px;'>Loading...</div>").insertAfter($("#submit_add_comment"));
    ...
    $email.on("change", function () {
        ...
        if (error_to_display) {
            $(".providerinfo").before('<div class="notice_error">' + error_to_display + '</div>');
        }
        if (message_to_display) {
            $(".providerinfo").before('<div class="notice_success">' + message_to_display + '</div>');
        }
    ...
    $phone.on("change", function () {
        ...
        let phone_val = $phone.val($phone.val().replace(/ /g, ""));
        let regexPhone = /^((\(\d{3}\))|(\d{3}))\s?-?\d{3}-?\d{4}$/;  //(979)388-0388 || (979) 778-0978 || 281-342-2452 || 1334431660
        if (phone_val.val().length > 3) {

            $(".notice_success").remove();
            $(".notice_error").remove();
            if (regexPhone.test(phone_val.val().replace(/-/g, ''))) {
                $email.removeClass("hightlight_required");
                $phone.removeClass("hightlight_required");
                $phone2.removeClass("hightlight_required");
        ...
        if (error_to_display) {
            $(".warningMessages").html('<div class="notice_error">' + error_to_display + '</div>');
        }

        if (message_to_display) {
            $(".warningMessages").html('<div class="notice_success">' + message_to_display + '</div>');
            if($phone.attr("rel") !== 'ok') {
    ...
    $phone2.on("change", function () {
        ...
        if ($phone2.val().length > 3) {
            ...
            let regexPhone = /^((\(\d{3}\))|(\d{3}))\s?-?\d{3}-?\d{4}$/;

            if (regexPhone.test($phone2.val().replace(/-/g, ''))) {
                $email.removeClass("hightlight_required");
                $phone.removeClass("hightlight_required");
                $phone2.removeClass("hightlight_required");
                loading('Check the phone number');
        ...
        if (error_to_display) {
            $(".warningMessages").before('<div class="notice_error">' + error_to_display + '</div>');
        }

        if (message_to_display) {
            $(".warningMessages").html('<div class="notice_success">' + message_to_display + '</div>');
            if($phone2.attr("rel") !== 'ok') {
                $("input[name='submit_escort']").hide();
            }
        } else {
            $("input[name='submit_escort']").show();
...
//SUBMIT FORM
    $("#add_an_escort input[name='submit_escort']").on("click", function (e) {
        ...
        let $data_to_send = "";
        let error_to_display = "";
        const regexPhone = /^((\(\d{3}\))|(\d{3}))\s?-?\d{3}-?\d{4}$/; //(979)388-0388 || (979) 778-0978 || 281-342-2452 || 1334431660
        ...
        if ($phone.length && $phone.val().length > 0) {
            if (regexPhone.test($phone.val().replace(/-/g, ''))) {
                $.ajax({
                    type: "POST",
                    async: false,
                    url: "/controller/ajax/verify_phone.php?" + ($("#agency_name").val()?.length < 2 ? 'just_phone=1' : ''),
        ...
        if (!$isOK) {
            ...
            $("<div style='float:right; font-size:16px;'>Loading...</div>").insertAfter($(this));
            $(this).hide();
        ...
        if (!$isOK) {
            ...
            if (top != 0) {
                body.animate({scrollTop: 0}, '1000', function () {
                    //DO SOMETHING AFTER SCROLL ANIMATION COMPLETED
                    $(".notice_error").remove();
                    $(".users_body").before('<div class="notice_error">' + error_to_display + '</div>');
                });
            }
    ...
    $('.bbnnrr').append('<span class="mobile-toogle-banner"></span>');
...
function checkShareFriends() {
    ...
    if (!(friend_email && isValidEmailAddress(friend_email))) {
        $('<div class="error_lbox">Please insert a valid email</div>').insertAfter(".pp_inline #share_friends_form");
        $err++;
    }

    if (email_content.length < 20) {
        $('<div class="error_lbox">Please insert a minimum 20 chars</div>').insertAfter(".pp_inline #share_friends_form");
        $err++;
    }
    ...
    $.widget("custom.catcomplete", $.ui.autocomplete, {
        ...
        _renderItem: function (ul, item) {
            return $("<li>")
                ...
                .append('<a href="' + (item.url ? item.url : "javascript:;") + '" class="" >' + (item.thumb ? '<span class="thumb" style="background-image:url(' + item.thumb + ');"></span>' : '') + item.label + (item.details ? '<span class="details">' + item.details + '</span>' : '') + '</a>')
                .appendTo(ul);
        },
        _renderMenu: function (ul, items) {
            ...
            $.each(items, function (index, item) {
                ...
                if (item.category != currentCategory) {
                    ul.append("<li class='ui-autocomplete-category ui-autocomplete-category-" + item.category.replace(/\W/g, '').toLowerCase() + "'>" + item.category + "</li>");
            ...
            success: function (data) {
                ...
                if (RESPONSE.STATUS == 'ERROR') {
                    ...
                    if (RESPONSE.CODE == 'username_exists') {
                        $("#" + formID + " .suggested_username").html('Available: <a href="javascript:;">' + RESPONSE.ADDITIONALDATA.suggested_username + '</a>').show();
...
$(document).ready(function () {
    ...
    const enablepayment = $("body").data("enablepayment");
    if (enablepayment) {
        var login_not_member = '<p style="font-size:16px;">Not a EM Member? <a href="/signup">Sign Up now, it\'s <b>FREE</b>!</a></p>\ ';
    } else {
        var login_not_member = '<p style="font-size:16px;">Not a EM Member? <a class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-signup">Sign Up now, it\'s <b>FREE</b>!</a></p>\ ';
    }

    $("body").append('\
		<div id="popup-signup" class="popupmodal">\
			...
			<div class="popupmodal-container">\
				<div class="popupmodal-header">\
					...
					Are you an escort? <a class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-signup-provider" >Sign-up Here</a>\
				</div>\
				<div class="popupmodal-body">\
					<div class="popupmodal-notice"></div>\
					<div class="info_nrreviews">Get instant access to a lot of user submited reviews, all over USA.</div>\
					<form role="form" method="post" id="signup_form_pop1" class="signup_form_popup" autocomplete="off">\
						<div class="form-group">\
							<input type="email" name="email" class="email form-control" placeholder="E-Mail">\
							<div class="email-info"><b>This e-mail will be displayed on your EM public profile</b></div>\
						...
						<div style="clear"></div>\
						<div style="text-align: center;"><button type="submit" class="btn btn_green alignnone">Free User Signup</button></div>\
						<br />\
						<div style="text-align: center;">\
							<p style="font-size:12px;">By creating an account, you agree to Erotic Monkey\'s <a href="/terms" target="_blank">Terms of Service</a> and <a href="/privacy" target="_blank">Privacy Policy</a>.</p>\
							<p>Already a Member? <a class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-login">Log In</a></p>\
						</div>\
...
\
		<div id="popup-signup-provider" class="popupmodal">\
			<a class="popupmodal-close" href="javascript:;"></a>\
			<div class="popupmodal-container">\
				<div class="popupmodal-header">\
					<h6>Escort Sign-up for EM! <b>100% FREE</b></h6>\
					Are you an user? <a class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-signup">Sign-up Here</a>\
				</div>\
				<div class="popupmodal-body">\
					<div class="popupmodal-notice"></div>\
					<div class="info_nrreviews">Get instant access to a lot of user submited reviews, all over USA.</div>\
					<form role="form" method="post" id="signup_form_pop2" class="signup_form_popup" autocomplete="off">\
						<div class="form-group">\
							<input type="email" name="email" class="email form-control" placeholder="E-Mail">\
							<div class="email-info"><b>This e-mail will be displayed on your EM public profile</b></div>\
							<div class="email_error register_error"></div>\
						...
						<div style="clear"></div>\
						<div style="text-align: center;"><button type="submit" class="btn btn_green alignnone">Free Escort Signup</button></div>\
						<br />\
						<div style="text-align: center;">\
							<p style="font-size:12px;">By creating an account, you agree to Erotic Monkey\'s <a href="/terms" target="_blank">Terms of Service</a> and <a href="/privacy" target="_blank">Privacy Policy</a>.</p>\
							<p>Already a Member?  <a class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-login">Log In</a></p>\
...
\
		<div id="popup-login" class="popupmodal">\
			<a class="popupmodal-close" href="javascript:;"></a>\
			<div class="popupmodal-container">\
				<div class="popupmodal-header">\
					<h6>Login to Erotic Monkey!</h6>\
				</div>\
				<div class="popupmodal-body">\
					<div class="popupmodal-notice"></div>\
					<form role="form" id="login_form_popup" method="post" action="/users-sign-in">\
						<div class="form-group">\
							<input type="text" name="email" class="form-control InputEmail" placeholder="Enter email / username">\
						</div>\
						<div class="form-group">\
							<input type="password" name="password" class="form-control InputPassword" placeholder="Password">\
						</div>\
						<div class="form-group" id="mfa_code_container" style="display:none;">\
							<input type="text" name="mfa_code" class="form-control" id="mfa_code" placeholder="Mfa code"/>\
						</div>\
						<a class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-forgotpassword">Forgot Password?</a>\
						<button type="submit" class="btn btn_green alignright">Login</button>\
						<div style="clear"></div>\
						<br/>\
					</form>\
						<div style="text-align: center;">\
							' + login_not_member + '\
							<p style="font-size:12px;">Signing up to the Erotic Monkey gives you access to thousands of reviews from across the country. Find the latest reviews on the provider you\'re thinking of choosing before you waste your money.</p>\
						</div>\
				</div>\
			</div>\
		</div>\
\
		<div id="popup-forgotpassword" class="popupmodal">\
			<a class="popupmodal-close" href="javascript:;"></a>\
			<div class="popupmodal-container">\
				<div class="popupmodal-header">\
				...
				<div class="popupmodal-body">\
					...
					<form role="form" id="reset_form_popup" method="post">\
						<div class="form-group">\
							<input type="email" name="email" class="form-control InputEmail" placeholder="Enter email">\
						</div>\
						<div class="form-group">\
							<div class="cf-turnstile my-2 text-center" data-sitekey="0x4AAAAAABfDMXn6mmho-vTW"></div>\
						</div>\
						<a class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-login">Back to login</a>\
						<button type="submit" class="btn btn_green alignright">Submit</button>\
						<div style="clear"></div>\
						<br/>\
					</form>\
				</div>\
...
\
		<div style="display:none;">\
			<a id="trigger-openpopupmodal-login"  class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-login">login</a>\
			<a id="trigger-openpopupmodal-signup" class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-signup">signup</a>\
			<a id="trigger-openpopupmodal-signup-provider" class="openpopupmodal" href="javascript:;" data-openpopupmodal="#popup-signup-provider">signup</a>\
		</div>\
\
');
    ...
    function layoverHashCheck() {
        if (window.location.hash) {
            switch (window.location.hash) {
                ...
                case '#signupforfree':
                    $('#trigger-openpopupmodal-signup').trigger("click");
                    $('.info_nrreviews').html("<span style='font-size:13px'><b>Your free session has expired</b>, please sign up to <b>get unlimited access, 100% free!</b></span>");
                    break;
                case '#signup-provider':
                    $('#trigger-openpopupmodal-signup-provider').trigger("click");
                    break;
            }
        }
    }
    ...
    $("#signup_form_pop1").validate({
        ...
        messages: {
            ...
            'cf-turnstile-response': {required: "Please complete captcha verification"}
        },
        ...
        showErrors: function (errorMap, errorList) {
            ...
            if ((errorList.length !== 0)) {
                ...
                for (index = 0; index < errorList.length; ++index) {
                    ...
                    TrackValidateErrors.push({
                        CATEGORY: 'Join',
                        NAME: this_.element.name.toUpperCase(),
                        MESSAGE: this_.message,
                        VALUE: this_.element.value
                    });
                }
            }
        },
        ...
        submitHandler: function (form) {
            ...
            if (err !=- 'error' && err2 !=- 'error') {
                $("#" + popupID).append('<div class="loading"><div class="loading-icon"></div></div>');
                $.ajax({
                    type: "POST",
                    url: "/signup",
                    ...
                    async: false,
                    success: function (data) {
                        $("#" + formID).closest('.popupmodal-container').html('<div class="popupmodal-body">' + data + '</div>');
    ...
    $("#login_form_popup").validate({
        ...
        submitHandler: function (form) {
            ...
            let data = "login=1&email=" + $("#login_form_popup input[name=email]").val() + "&password=" + $("#login_form_popup input[name=password]").val();
            const mfaCode = $("#login_form_popup #mfa_code_container #mfa_code").val();
            if(mfaCode) {
               data += '&mfa_code='+mfaCode;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data === '/addescort') {
                        ...
                        var http_referer = getCookie('http_referer');
                        if (http_referer && http_referer.indexOf('https') === -1) {
                            deleteCookie('http_referer');
                            window.location.href = http_referer;
                        } else {
                            window.location.reload();
                        ...
                        $("#" + popupID + ' .popupmodal-notice').html('<div class="alert-popup alert-error">Your account is banned!</div>').show();
                    } else if (data === 'invalid_credentials') {
                        $("#" + popupID + ' .popupmodal-notice').html('<div class="alert-popup alert-error">The email/username or password is incorrect</div>').show();
                    } else if (data === 'notconfirmed') {
                        $("#" + popupID + ' .popupmodal-notice').html('<div class="alert-popup alert-error" style="text-align: center;">Please check the registration email and confirm your account! <br /> <span style="color:#000000;">or</span> <br /> <a href="/users-resend-confirm">Resend the confirmation code!</a></div>').show();
                    } else if (data === 'mutipleLoginAttempts') {
                        $("#login_form .register_error").remove();
                        $("#" + popupID + ' .popupmodal-notice').html('<div class="alert-popup alert-error">You tried to login multiple times in the last hour. You should reset your password or wait 1 hour until you will be able to try again.</div>').show();
                    } else if (data === 'mfa_missing' || data === 'mfa_wrong') {
                        $("#login_form .register_error").remove();
						$("#login_form_popup #mfa_code_container").show();
                        const text = data === 'mfa_missing' ? 'Mfa code is missing.' : 'Mfa code is wrong.';
                        $("#" + popupID + ' .popupmodal-notice').html(`<div class="alert-popup alert-error">${text}</div>`).show();
                    } else if (data === 'isbannedProvider') {
                        /*$("#"+popupID+' .popupmodal-notice').html('<div class="alert-popup alert-error">Your account has been temporarily disabled due to an increase of fraud profiles. Please submit a photo of you holding a paper with today\'s date written, make sure you have 3 fingers over the paper, after the verification process we will enable your account.\
                        <form action="/" method="post" id="form_user_escortprofile_verify" enctype="multipart/form-data">\
                                        <input type="file" name="image_verify" style="margin:15px 0; " accept="jpeg|jpg|gif|png" />\
                                        <input type="submit" name="submit_escort_verify" class="btn " value="Send" style="padding:5px 15px;" />\
                                        <div class="clear"></div>\
                                    </form>\
                        ...
                        var buttonsss = '<a class="btn mt-2 mb-2" data-show=".verify_profile_video">Video Verification</a><a class="btn mt-2 mb-2" data-show=".verify_profile_photo">Photo Verification</a>';

                        if (isMobileee) {
                            buttonsss = '<a class="btn mt-2 mb-2" data-show=".verify_profile_video">Video Verification</a>';
                        } else {
                            buttonsss = '<a class="btn mt-2 mb-2" data-show=".verify_profile_photo">Photo Verification</a>';
                        }

                        $("#" + popupID + ' .popupmodal-notice').html('\
							<div class="verify_profile_intro">\
							<div class="notice_success_container">\
								' + texttt + '\
								<div class="clear"></div>\
								' + buttonsss + '\
								<div class="clear"></div>\
	...
	\
						<div class="verify_profile_photo	p-2 mb-3">\
							<div>Please submit a photo of you holding a paper with today\'s date written, make sure you have 3 fingers over the paper.\
							<form action="/" method="post" id="form_user_escortprofile_verify" enctype="multipart/form-data">\
							<input type="file" name="image_verify" style="margin:15px 0; " accept="jpeg|jpg|gif|png" />\
							<input type="submit" name="submit_escort_verify" class="btn " value="Send" style="padding:5px 15px;" />\
							<div class="clear"></div>\
	...
	\
						<div class="verify_profile_video	pt-2 mb-3">\
							<div class="iframegoeshere"></div>\
						</div>').show();
                        ...
                        $("#" + popupID + ' .popupmodal-notice').html('<div class="alert-popup alert-error">Your photo/video has been succesfully uploaded, our admins will start the verification process ASAP.</div>').show();
                        $("#login_form_popup").next().remove();
                        $("#login_form_popup").remove();
                        $("#" + popupID + ' .popupmodal-notice').removeClass('popupmodal-notice').addClass('popupmodal-notice-lock'); // display permanent
                    }
